...
## Usage
## python transition_time_TM_conservative.py [labelled newick file] > [output file]  

## This script differs from transition_time_TM.py in that it is more conservative in what it counts as a transition.
...
for node in t.search_nodes(Env="M"):
    ...
    if not node.is_root() and not node.is_leaf():	
        ...
        if getattr(parent, "Env") == "T":
            print "T_to_M\t%s\t%s" % (depth, rel_depth)
